...
nohup $JBOSS_HOME/bin/domain.sh -DSERVER=$SERVER_NAME -P=$DOMAIN_BASE/$SERVER_NAME/bin/env.properties -b $BIND_ADDR --backup >> $LOG_HOME/nohup/$SERVER_NAME.out 2>&1 &
#nohup $JBOSS_HOME/bin/domain.sh -DSERVER=$SERVER_NAME -P=$DOMAIN_BASE/$SERVER_NAME/env.properties -b 0.0.0.0 --cached-dc >> $LOG_HOME/nohup/$SERVER_NAME.out 2>&1 &
